...
Читатель заводится сам: пишет боту `/start` и получает одноразовую ссылку
на сайт. Сбор и оценка идут один раз на всех — они не зависят от того, кто
читает, — а отбор, текст и статистика персональны.
...
контейнере. Ключи моделей и источников читает один пайплайн.
...
src/lib/
  readers.ts        читатели, их темы, счёт расходов
