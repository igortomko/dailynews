        ...
        <div className="ml-auto flex shrink-0 items-center gap-0.5">
          ...
          <DropdownMenu defaultOpen>
            ...
            {/* Ширина по самой длинной строке: иначе меню жмётся к кнопке
                ...
                Это `w-max`, а не `min-w-max` поверх `w-(--anchor-width)`:
                при min-width коробка меню на момент раскладки шириной
                с саму кнопку (40px), позиция считается по ней, а потом
                содержимое распирает её вправо — за край экрана. На телефоне
                меню так и открывалось наполовину за экраном, и обрезанными
                оказывались как раз подписи, ради которых оно и сделано.
                `max-w-(--available-width)` — предохранитель: до края
                экрана и ни пикселем дальше. */}
            <DropdownMenuContent align="end" className="w-max max-w-(--available-width)">
              {/* На тапе действия живут только здесь, поэтому «Черновик поста»
