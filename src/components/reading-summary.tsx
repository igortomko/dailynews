...
import { relationText, stepStateText } from "@/lib/reading-document";
import { typography as t } from "@/lib/typography";

function Block({ block: b }: { block: ReadingBlock }) {
  switch (b.kind) {
    ...
    case "flow": return <div className="flex flex-wrap items-center gap-x-4 gap-y-3 rounded-xl bg-muted/60 p-4">
      {b.nodes.map((node,i) => <div key={i} className="flex min-w-0 items-center gap-4">
        {i > 0 ? <span className="text-muted-foreground" aria-label={b.relations[i-1] === "equivalent" ? "равно" : "затем"}>{relationText[b.relations[i-1]]}</span> : null}
    ...
    case "steps": return <ol className="space-y-3">{b.items.map((item,i) => <li key={i}>
      <p className="text-sm text-muted-foreground">{b.sequence === "procedure" ? "Шаг" : "Этап"}{'\u00a0'}{i+1}{stepStateText[item.state] ? ` · ${stepStateText[item.state]}` : ""}</p>
...
export function ReadingSummary({ reading }: { reading: StoredReading }) {
