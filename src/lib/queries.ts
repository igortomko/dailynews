 ...
 * их в календаре и получил одну ленту вместо трёх.
 *
 * Один сюжет — одна карточка, и сводится он здесь, а не только в отборе.
 * Отбор честно исключает уже уходившее по сюжету (`coalesce(dup_of, id)`
 * в `pipeline/select.ts`), но по тому, что знал на момент отбора, а связь
 * `dup_of` ставится после сбора и позже самого выпуска: третий слой дедупа —
 * вопрос к Jev — отвечает и на следующем прогоне, а упавший вопрос
 * повторяется завтра. Замер на живой базе 22 сентября 2026: четыре пары
 * карточек, у всех `dup_asked_at` — 20 сентября 13:59, то есть на сутки
 * позже своих выпусков; три пары попали в один день (19 сентября), одна
 * разъехалась по двум. Пересобрать выпуск нечем — он уже ушёл в Telegram
 * и книгой на читалку, — поэтому повтор снимается при показе.
 *
...
export async function getFeed(
  ...
  >`
    ...
    cards as (
    -- Один сюжет — одна карточка. Ключ тот же, что у отбора и у витрины
    -- публикаций: coalesce(dup_of, id). Дедуп доезжает позже выпуска,
    -- и без этой строки одна новость приходила бы дважды — в одном дне
    -- или в соседних днях окна.
    --
    -- Представитель — первый в порядке самой ленты (total desc, id).
    -- Второй порядок для того же списка однажды разъехался бы с первым,
    -- и отсечка по времени резала бы не тот хвост. Отброшенная публикация
    -- не пропадает: getStories находит её по тому же ключу сюжета,
    -- и карточка называет её строкой «о том же написали ещё N твоих
    -- источников» — ровно то, что произошло.
    select distinct on (coalesce(i.dup_of, i.id))
           i.id, i.url, i.title, i.excerpt, di.title as title_ru, di.summary, di.summary_document, i.image_url,
 ...
 left join dailynews.topics t on t.id = sc.topic_id
     ...
     -- Порядок здесь выбирает представителя сюжета, а не выдачу: наружу
     -- лента всё равно уходит отсортированной ниже.
     order by coalesce(i.dup_of, i.id), di.total desc, i.id
    )
    ...
    --
    -- Сюжеты сведены раньше — в самой cards, — и это тоже не порядок
    -- ради порядка: повтор, снятый после отсечки, всё равно съел бы
    -- чужое место в заказанных минутах, а «ещё N не влезло» посчитало бы
    -- его за карточку.
    select *
      from (
        select *,
