...
export const feed: typeof En = {
  ...
  minutes: {
    ...
    // Число на самой кнопке: она стоит под лентой, а строка, которая его
    // назвала, — над первой карточкой, и возвращаться к ней глазами
    // за ответом «сколько ещё» читатель не должен.
  ...
  item: {
    ...
    opinion: "Черновик поста",
    opinionAria: "Черновик поста твоим голосом",
    opinionTooltipReady: "Два варианта черновика для Telegram, X, LinkedIn, Threads",
    // Имя тарифа приходит из `cheapestFor`, а не написано в строке:
    // Kindle уже переезжал с Pro на Plus, и подпись, живущая отдельно
    // от правила, после такого переезда врёт — в единственном месте,
    // где читатель спрашивает, сколько это стоит.
    opinionTooltipLocked: (plan: string) => `Черновик поста — на тарифе «${plan}»`,
    pickNetworksFirst: "Сначала отметь, где ты публикуешь",
    kindleSending: "Отправляю…",
    kindleSent: "Уже на Kindle",
    kindleSend: "Отправить на Kindle",
    kindleAria: "Отправить на Kindle",
    kindleTooltip: "Отправить статью на Kindle",
    kindleTooltipLocked: (plan: string) => `Kindle — на тарифе «${plan}»`,
    share: "Поделиться",
