...
export const errors: typeof En = {
  ...
  // Сумма предела ничего не говорит о том, сколько ушло и когда отпустит:
  // «до завтра» у потолка наступает в полночь UTC, а не в полночь читателя.
  dailyCap: (usd: number, spent: number, resetIn: string) =>
    `Потрачено $${spent.toFixed(2)} из $${usd} за сутки. Потолок обновится через ${resetIn}`,
