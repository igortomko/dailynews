...
export const settings: typeof En = {
  ...
  personalization: {
    ...
    aboutHint: "Чем подробнее напишешь, тем точнее мы отберём важные тебе новости.",
  ...
  delivery: {
    general: "Выпуск собирается в 02:00 по твоему поясу. Куда он приходит — решают направления ниже.",
    telegram: {
      description: "Выпуск сообщением: заголовки по темам и ссылка на каждую статью.",
      connected: (username: string | null) => `Подключён${username ? ` как @${username}` : ""}.`,
      connect: "Подключить Telegram",
      connectHint: "Откроется бот — нажми «Старт», и выпуск начнёт приходить и в Telegram.",
      noBot: "Бот сейчас недоступен — попробуй позже.",
      change: "Сменить аккаунт",
      remove: "Отвязать Telegram",
      removed: "Telegram отвязан",
      onlyWay: "Telegram не отвязать: это твой единственный вход. Сначала добавь почту",
      changeHint: "Откроется бот — нажми «Старт» в том аккаунте, куда присылать выпуск. Прежний отвяжется сам.",
      podcast: "Выпуск голосом",
      podcastHint: "Запись выпуска: в Telegram приходит вместе с сообщением, в письме — ссылкой «Слушать».",
    ...
    email: {
      title: "Почта",
      description: "Выпуск письмом: тот же список заголовков и ссылка на ленту.",
      address: (email: string) => `Адрес: ${email}.`,
      digest: "Присылать выпуск письмом",
      digestHint: "Каждое утро, в одно время с Telegram.",
      digestOn: "Письма включены",
      digestOff: "Письма выключены",
      label: "Адрес почты",
      placeholder: "you@example.com",
      confirm: "Подтвердить адрес",
      sent: (email: string) => `Отправили письмо на ${email} — нажми в нём «Подтвердить».`,
      change: "Сменить адрес",
      cancel: "Отмена",
      remove: "Убрать почту",
      removed: "Почта убрана",
      invalid: "Это не похоже на адрес почты",
      same: "Этот адрес уже подключён",
      taken: "Этот адрес — вход другого профиля Reporta",
      tooSoon: "Письмо уже ушло — подожди минуту",
      failed: "Не получилось отправить письмо. Попробуй через минуту",
      noAddress: "Сначала добавь адрес",
      onlyWay: "Почту не убрать: это твой единственный вход. Сначала подключи Telegram",
      confirmed: "Почта подтверждена — выпуск будет приходить и письмом.",
      confirmTaken: "Этот адрес уже вход другого профиля Reporta.",
      confirmExpired: "Ссылка подтверждения устарела — отправь её ещё раз.",
    },
    kindle: {
