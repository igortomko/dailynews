...
export const sql = postgres(url, {
  ...
  // Наши id и счётчики в безопасный диапазон числа помещаются с запасом,
  // а строка ломается тихо: "136" !== 136, и сравнение с любым числом,
  // пришедшим со стороны (от модели, из JSON, из параметра), не совпадает
  // никогда. Один такой разнобой обнулил перевод всего дайджеста.
  types: {
    int8: {
      to: 20,
      from: [20],
