 ...
 * и при сумме целей, равной размеру дайджеста, каждая тема получает
 * ровно свою цель. Это единственная причина, по которой интерфейс может
 * показывать штуки, а не проценты.
 *
...
/**
 * Сколько новостей в день можно заказать. Список, а не поле ввода: шаг в
 * двадцать — это заметная разница в том, сколько читать, а «37» такой
 * разницы не несёт и только требует решения.
 *
 * Потолок совпадает с ограничением колонки digest_size (миграция 0018).
 * Разъедется — форма предложит число, которое база отвергнет.
 */
export const DIGEST_SIZES = [20, 40, 60, 80, 100];
export const MAX_DIGEST = DIGEST_SIZES[DIGEST_SIZES.length - 1];
