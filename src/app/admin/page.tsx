...
/**
 * Вкладки, а не одна длинная страница: продуктовую часть рисует
...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ tab?: string | string[] }> }) {
  ...
  const tab = raw === "costs" || raw === "links" ? raw : "product";
  return (
    <>
      <nav className="mx-auto flex max-w-[1160px] items-center gap-1 px-3 pt-4 text-sm sm:px-6" aria-label="Разделы дашборда">
        ...
        <TabLink href="/admin?tab=links" active={tab === "links"}>Ссылки</TabLink>
      ...
      {tab === "costs" ? <CostsView costs={await loadCosts()} /> : tab === "links" ? <LinksView /> : <ProductDashboard />}
    </>
  );
}
