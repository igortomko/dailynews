 ...
 */
const PITCH = [
  "Лента читает за тебя блоги, каналы и рассылки, а утром присылает короткий выпуск: только то, что стоит прочитать.",
  "",
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (command.kind === "link") {
      ...
      after(async () => {
        try {
          ...
          await sendMessage(chatId, "Не получилось прочитать ссылку — попробуй ещё раз.").catch(() => {});
        }
      });
    ...
    const reader = await ensureReader(
      command.telegramId, command.username, command.locale,
      command.kind === "start" ? command.source : null,
    );
