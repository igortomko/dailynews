...
async function main() {
  ...
  try {
    ...
    // Лента отдаёт длину текста статьи. Колонка, заведённая миграцией,
    // но не выбранная запросом, ничем себя не выдаёт — поэтому она
    // проверяется здесь, а не глазами.
    //
    // Во что она превращается, не спрашивается: карточка считает время
    // по `summaryTime` из длины нашего описания, и `body_chars` сейчас
    // не читает никто. Утверждение про минуты проверяло бы функцию,
    // которой в ленте уже нет. Судьба самой колонки — отдельной задачей.
