...
Карточка должна улучшать его посты, а не копировать слабые: "structure"
и "hooks" собирай по постам ВЫШЕ медианы (из тех, по которым собираешь
форму). Приёмы, которые встречаются в основном ниже медианы, в "structure"
и "hooks" не бери, а впиши в "taboo" с пометкой «(в слабых постах)».
Голос и словечки — по всем: их не улучшают, по ним его узнают.`;

const PROMPT_UNRANKED = `"frame" — оставь пустым массивом: статистики у этих постов нет,
а каркас без неё был бы догадкой, выданной за наблюдение. Каркас из данных
чужих каналов код добавит сам.`;

/**
 * Каркас из корпуса — когда своей статистики нет: вставленные посты,
 * LinkedIn и Threads без просмотров, канал моложе медианы. Без него форма
 * копировалась бы со всех постов подряд, вместе со слабыми, и улучшать
 * было бы нечем. Числа — `tg-viral-post` и `news-take` (962 поста верхних
 * тиров десяти авторских каналов, 304 на чужом материале): это чужие каналы,
 * и так и сказано, — своя статистика, когда появится, главнее.
 */
export const CORPUS_FRAME = [
  "первая строка несёт число, признание или вердикт, а не название темы или компании: хук с числом в верхнем тире в 47% случаев, признание — в 54%, «Компания выпустила…» — в 30%",
  "цифры, имена и даты вместо общих слов: конкретика почти вдвое поднимает пересылки",
  "длина — либо реакция до 300 знаков, либо разбор 900–1500: середина 300–900 хуже обоих",
  "мнение — первой строкой или ставкой в конце, а не приклеенной строкой после пересказа",
  "конкретная просьба или никакой: «что думаете?» и «буду благодарен за реакции» — худшие концовки",
];
...
короткого текста без заголовков и списков.`,
    ...
    card.frame.length
      ...
      : card.built_from > 0
        ? `Своей статистики у него нет, поэтому форму подтягивай к тому, что заходит в авторских каналах (данные чужих каналов, голос не трогай):${list(CORPUS_FRAME)}`
        : "",
    card.samples.length
...
export function cardText(card: VoiceCard): string {
  ...
  return [
    ...
    card.frame.length
      ...
      : section("Что заходит в авторских каналах (своей статистики пока нет):", card.built_from > 0 ? CORPUS_FRAME : []),
  ].filter(Boolean).join("\n\n");
}
