...
function questions(readerContext: string) {
  return {
    ...
    // Вопрос переписан после первого измерения. Прежний — «сказано ли,
    // чем это касается именно его» — засчитывал за связь само упоминание
    // читателя, и «Datasette может быть полезен Игорю для дашбордов»
    // получало единицу. Читатель на такие описания отвечал «не понял,
    // зачем мне это»: ось поощряла ровно ту форму, которую он отвергал,
    // и правки промпта по ней вели не туда. Сравнивать числа до и после
    // этой строки нельзя — это разные вопросы.
    reader_relevance: noul(
      `Читатель: ${readerContext}\n\nМожет ли он после этого описания что-то сделать или решить иначе?`,
      {
        true: "названо действие, проверка или решение: что заменить, что перепроверить у себя, какой приём повторить, чего перестать ждать",
        false: "только сообщение о событии либо объявленная польза без действия — «может быть полезен», «стоит учитывать», «важно для его работы»",
      },
    ),
