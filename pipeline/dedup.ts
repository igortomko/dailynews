...
/**
 * Свести цепочки `dup_of` к корню.
 *
 * Весь остальной код считает сюжет выражением `coalesce(dup_of, id)` —
 * то есть верит, что повтор указывает прямо на корень. Ничем это
 * не обеспечено. `askDuplicates` строит шортлист один раз, до пометок,
 * и пишет выбранный id, не перепроверяя, остался ли тот корнем: пока
 * отвечает вопрос про Y, кандидат X успевает стать повтором W. Первый
 * слой умеет то же самое — внутри одного `update` все строки видны такими,
 * какими были до него, поэтому B→A и C→B проставляются заодно.
 *
 * Цепочка не падает, она делит сюжет надвое: у карточки корня в списке
 * двое, у дальнего звена — никого, и счётчик «ещё N источников»
 * недосчитывает. Хуже того, ключом кластера становится середина, у которой
 * оценки может не быть вовсе, — и материал уходит из отбора молча.
 * В живой базе такая цепочка уже лежала: 2231 → 2035 → 26, три публикации
 * одного сюжета про галлюцинацию ИИ в военной разведке.
 *
 * Выпрямляется на месте, а не при чтении: читателей у этого выражения три
 * (лента, отбор, копия оценки), и рекурсия в каждом из них разъехалась бы
 * с остальными. Циклов быть не может — повтор всегда указывает на меньший
 * id, — но предел шагов стоит: инвариант держится кодом дедупа, а не базой,
 * и если его однажды нарушат, вечный цикл в ночном прогоне будет худшим
