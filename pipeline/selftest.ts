...
// Ручкой время становится только на окне: на одном дне выпуск уже собран
// под заказ читателя из настроек, и меню над ним режет то, что и так
// заказано. Заданный заказ держит ручку и на одном дне — иначе снять
// предел, переживший переход стрелкой, было бы нечем.
assert.match(
  headerRow,
  /\{days > 1 \|\| minutes !== null \?\s*\(?\s*<MinutesSelect/,
  "ручка заказа показывается на окне или когда заказ уже задан",
);
assert.match(
  headerRow.slice(headerRow.indexOf("<MinutesSelect")),
  /formatMinutes\(reading\.minutes/,
  "а на обычном дне время остаётся подписью, а не исчезает из шапки",
);
