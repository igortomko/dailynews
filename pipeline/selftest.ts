...
// Подписи у делений и манер не повторяются: две одинаковые строки в ряду
// означают, что выбирать не из чего, — а деления при этом разные.
assert.equal(
  new Set([...COMPLEXITY, ...STYLES].map((entry) => entry.hint)).size,
  COMPLEXITY.length + STYLES.length,
  "у каждого варианта своя подпись: повтор означает неразличимые соседи",
);
