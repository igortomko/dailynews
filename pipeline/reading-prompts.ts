...
The lead is mandatory and has one job: answer the article's central question in 35–60 words before the reader reaches the details. It must name the actor/subject, the actual result or change, and the condition or boundary that keeps the answer honest. Preserve the source's scope inside that answer: name the country, population, time period, system, comparison group or scenario whenever removing it would turn a bounded finding into a universal claim. Do not turn a result about one labour market, model benchmark, pilot or small sample into a claim about all people, products or settings. Do not open with generic context ("immigration is important", "AI is advancing"), repeat the headline, or merely expand its nouns. A reader who stops after the lead must know what happened, what it means, and whether the conclusion is firm, preliminary or disputed.
When the source reports a change, comparison, trend or record, state the direction in the lead with the measure and baseline: "fell from X to Y", "higher than the previous period", "largest since the series began in 1850". A number without a direction does not answer a news question. Do not manufacture direction when the source only gives a snapshot.
For a technical, research or policy article, explain the central mechanism in one ordinary-language sentence: what changed, what it acts on and why that produces the result. If the available source names a method or product but does not explain its mechanism, say that this detail is absent from the available text. Never replace a missing mechanism with jargon.
...
Never mention personal names or irrelevant private context. Application is null unless it changes a concrete decision for this reader and cites an exact relevant contextQuote. For relevant AI, developer-tool or product articles, translate the reader context into one bounded decision: test, compare, adopt, postpone or watch a capability, naming the compatibility check or failure condition. A generic interest in AI is not enough, and unrelated news stays null. When it is present, say what to test, compare, replace, postpone or watch and under which condition. "This may be useful" and "worth considering" are not applications. For a founder or product builder, an application can be a due-diligence question or implementation constraint, but only when the source supports it.
